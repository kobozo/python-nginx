...
def test_state_directive():
    ...
    for ctx in good_contexts:
        crossplane.common.analyzer.analyze(fname, stmt, term=';', ctx=ctx)

    # the state directive should not be in any of these contexts
    bad_contexts = set(crossplane.common.analyzer.CONTEXTS) - good_contexts

    for ctx in bad_contexts:
        try:
            crossplane.common.analyzer.analyze(fname, stmt, term=';', ctx=ctx)
            raise Exception("bad context for 'state' passed: " + repr(ctx))
        except crossplane.common.errors.NgxParserDirectiveContextError:
            continue
...
def test_flag_directive_args():
    ...
    for args in bad_args:
        ...
        except crossplane.common.errors.NgxParserDirectiveArgumentsError as e:
